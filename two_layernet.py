...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        a_1 = X
        z_2 = np.dot(a_1, W1) + b1
        a_2 = np.maximum(0,z_2)
        z_3 = np.dot(a_2, W2) + b2 
        a_3 = (np.divide(np.exp(z_3).T, np.sum(np.exp(z_3), axis = 1))).T
        scores = a_3
        ...
        exp_func = exp_scores / np.sum(exp_scores, axis=1, keepdims=True)
        data_loss = np.sum(-np.log(exp_func[range(N), y])) / N

        loss = data_loss + reg*(np.linalg.norm(W1)**2+np.linalg.norm(W2)**2)
        ...
        # gradient of W1
        # Formula: (dz2/dW1)*(da2/dz2)*(dj/da2)
        dJ_dz3=(1/N)*(scores-Delta)
        dJ_da2=np.dot(dJ_dz3,W2.T)
        dJ_da2[z_2<0]=0
        # We get that, dz2/dw1=a_1
        grads['W1']=np.dot(a_1.T,dJ_da2)+(2*reg*W1)
        
        # gradient of W2
        grads['W2']=np.dot(a_2.T,dJ_dz3)+(2*reg*W2)
        
        # gradient of b1
        dJ_dz2=dJ_da2.copy()
        dJ_dz2[z_2<0]=0
        grads['b1']=np.sum(dJ_dz2, axis = 0)
        
        # gradient of b2
        grads['b2']=np.sum(dJ_dz3, axis = 0)
